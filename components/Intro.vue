...
const {
  ...
  refresh,
  error,
} = await useFetch("/api/listeningActivity", { method: "GET" }).catch((err) =>
  console.log(err)
);

onMounted(() => {
  setInterval(async () => {
    if (document && !document.hidden) await refresh();
  }, 2000);

  scroll(
    animate(".bottomLine", {
      translateY: ["0px", "1800px"],
      opacity: ["1", "0", "0", "0"],
      scale: ["1", "0.1", "0.2", "0"],
    })
  );
...
<template>
  <div class="intro">
    <div class="spotify animate">
      ...
      <div class="spotifyText hover" v-if="activity?.track">
        <span class="songStatus">Listening to</span>
        <a class="song" :href="activity.track.url" target="_blank">{{
          activity.track.name
        }}</a>
        <span class="songStatus">by</span>
        <a
          class="song hover"
          :href="activity?.artists[0].url"
          target="_blank"
          >{{ activity?.artists[0].name }}</a
        >
      </div>
    ...
    <div class="about">
      <div>
        <span class="line animate">Self-taught front-end</span>
        <span class="line animate">developer based in</span>
        <span class="line animate">Ankara, Turkiye.</span>
      </div>
...
.intro:before {
  ...
  pointer-events: none;
  animation: tingle 4s infinite ease-in-out;
}
...
.intro:after {
  ...
  top: 0;
  left: 0;
  background: url("~/assets/hexagons.png");
  background-size: cover;
  z-index: 100;
  opacity: 0.4;
  -webkit-mask-image: linear-gradient(to bottom, black 0%, transparent 50%);
